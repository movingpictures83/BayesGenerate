...
#	print len(alignmentParameters)
	...
	for line in infile:
		...
		for taxaIndex in range(0, len (taxaNames)):
	...
	for subjectID in subjectIDs:
		...
		for taxaName in abundanceByTaxa:
			abundanceLevelPerTimepoint = abundanceByTaxa[taxaName]
			timepoints = []
			relativeAbundances = []
	...
	for currentSampleID in subjectIDs:
		if currentSampleID == referenceSampleID:
			for currentTimepoint in timepointsReferenceSample:
				daysOfAntibiotics = int(round(float(interpolate.splev(currentTimepoint, referenceSampleDaysOfAntibioticsSplineParameters))))
				...
				for taxaName in referenceSample:
					abundanceLevelPerTimepoint = referenceSample[taxaName]
			...
			for currentTimepoint in timepointsCurrentSample:
				daysOfAatibiotics = int(round(float(interpolate.splev(currentTimepoint, currentSampleDaysOfAntibioticsSplineParameters))))
				...
				#Save the taxon relative abundance values for this timepoint 
				taxonAbundanceValues = {}
				for taxaName in currentSample:
					abundanceLevelPerTimepoint = currentSample[taxaName]
			...
			for referenceTimepoint in timepointsAlignedInterval:
				...
				daysOfAntibiotics = int(round(float(interpolate.splev(currentTimepoint, currentSampleDaysOfAntibioticsSplineParameters))))
				gestationalAgeAtBirth = gestationalAgePerSubject[currentSampleID]
				postconceptionalAgeSampleObtained = (currentTimepoint/7.0) + gestationalAgeAtBirth
				gender = genderPerSubject[currentSampleID]
				modeOfBirth = modeOfBirthPerSubject[currentSampleID]
...
import PyPluMA
import PyIO
class BayesGeneratePlugin:
    def input(self, infile):
        parameters = PyIO.readParameters(infile)
        self.annotated = PyPluMA.prefix()+"/"+parameters["annotated"]
        self.selected = PyPluMA.prefix()+"/"+parameters["selected"]
        self.alignment = PyPluMA.prefix()+"/"+parameters["alignment"]
    def run(self):
        pass
    def output(self, outputfile):
        main(['',self.annotated,self.selected,self.alignment,outputfile+"/noalignment",outputfile+"/alignment"])
